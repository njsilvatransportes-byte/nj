...
async function initializeDatabase() {
  await pool.query(`CREATE TABLE IF NOT EXISTS users (id UUID PRIMARY KEY, name TEXT NOT NULL, email TEXT UNIQUE NOT NULL, password_hash TEXT NOT NULL, approved BOOLEAN NOT NULL DEFAULT FALSE, created_at TIMESTAMPTZ DEFAULT NOW());
    ...
    CREATE TABLE IF NOT EXISTS clients (id UUID PRIMARY KEY, name TEXT NOT NULL, document TEXT, phone TEXT, address TEXT NOT NULL, number TEXT, neighborhood TEXT, city TEXT NOT NULL, state TEXT NOT NULL, zip_code TEXT, reference TEXT, status TEXT NOT NULL, created_at TIMESTAMPTZ DEFAULT NOW());
     CREATE TABLE IF NOT EXISTS freight_entries (id UUID PRIMARY KEY, date DATE NOT NULL, driver_id UUID REFERENCES drivers(id), vehicle_id UUID REFERENCES vehicles(id), client_id UUID REFERENCES clients(id), destination TEXT NOT NULL, freight NUMERIC(12,2) NOT NULL, tolls NUMERIC(12,2) DEFAULT 0, expenses NUMERIC(12,2) DEFAULT 0, created_at TIMESTAMPTZ DEFAULT NOW()); CREATE TABLE IF NOT EXISTS partner_stations (id UUID PRIMARY KEY, name TEXT NOT NULL, cnpj TEXT, phone TEXT, address TEXT, city TEXT, state TEXT, created_at TIMESTAMPTZ DEFAULT NOW()); CREATE TABLE IF NOT EXISTS fuelings (id UUID PRIMARY KEY, date DATE NOT NULL, vehicle_id UUID REFERENCES vehicles(id), mileage NUMERIC, price_per_liter NUMERIC, quantity_liters NUMERIC, driver_id UUID REFERENCES drivers(id), station_id UUID REFERENCES partner_stations(id), notes TEXT, created_at TIMESTAMPTZ DEFAULT NOW()); CREATE TABLE IF NOT EXISTS audit_log (id UUID PRIMARY KEY, user_id UUID REFERENCES users(id) ON DELETE SET NULL, user_name TEXT, action TEXT NOT NULL, resource TEXT NOT NULL, resource_id UUID, detail JSONB, created_at TIMESTAMPTZ DEFAULT NOW()); ALTER TABLE users ADD COLUMN IF NOT EXISTS approved BOOLEAN NOT NULL DEFAULT FALSE; ALTER TABLE users ADD COLUMN IF NOT EXISTS active BOOLEAN NOT NULL DEFAULT TRUE; ALTER TABLE users ADD COLUMN IF NOT EXISTS role TEXT NOT NULL DEFAULT 'Operador'; ALTER TABLE users ADD COLUMN IF NOT EXISTS photo TEXT; ALTER TABLE drivers ADD COLUMN IF NOT EXISTS zip_code TEXT, ADD COLUMN IF NOT EXISTS address TEXT, ADD COLUMN IF NOT EXISTS number TEXT, ADD COLUMN IF NOT EXISTS neighborhood TEXT, ADD COLUMN IF NOT EXISTS city TEXT, ADD COLUMN IF NOT EXISTS state TEXT; ALTER TABLE drivers ADD COLUMN IF NOT EXISTS photo TEXT; ALTER TABLE users ADD COLUMN IF NOT EXISTS pending_password_hash TEXT;`);
...
async function api(request, response, url) {
  ...
  const resource = route.match(/^\/api\/(drivers|vehicles|clients|freight_entries|partner_stations|fuelings)(?:\/([\w-]+))?$/); if (!resource) return false;
  ...
  const [ , table, id ] = resource; const fields = table === 'drivers' ? ['name', 'cpf', 'cnh', 'category', 'expiry', 'status', 'phone', 'zip_code', 'address', 'number', 'neighborhood', 'city', 'state', 'photo'] : table === 'vehicles' ? ['plate', 'model', 'type', 'year', 'renavam', 'status'] : table === 'freight_entries' ? ['date','driver_id','vehicle_id','client_id','destination','freight','tolls','expenses'] : table === 'fuelings' ? ['date','vehicle_id','mileage','price_per_liter','quantity_liters','driver_id','station_id','notes'] : table === 'partner_stations' ? ['name','cnpj','phone','address','city','state'] : ['name', 'document', 'phone', 'address', 'number', 'neighborhood', 'city', 'state', 'zip_code', 'reference', 'status'];

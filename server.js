...
async function api(request, response, url) {
  ...
  const [ , table, id ] = resource; const fields = table === 'drivers' ? ['name', 'cpf', 'cnh', 'category', 'expiry', 'status', 'phone', 'zip_code', 'address', 'number', 'neighborhood', 'city', 'state', 'photo'] : table === 'vehicles' ? ['plate', 'model', 'type', 'year', 'renavam', 'status'] : table === 'suppliers' ? ['name', 'trade_name', 'document', 'phone', 'email', 'address', 'zip_code', 'number', 'neighborhood', 'city', 'state', 'supplier_type'] : table === 'freight_entries' ? ['date','driver_id','vehicle_id','client_id','destination','freight','tolls','expenses','sest_senat','irrf','inss'] : table === 'fuelings' ? ['date','vehicle_id','mileage','price_per_liter','quantity_liters','driver_id','station_id','notes'] : table === 'partner_stations' ? ['name','cnpj','phone','address','city','state'] : table === 'maintenance_entries' ? ['entry_date','exit_date','vehicle_id','supplier_id','maintenance_type','category','description','cost','status'] : ['name', 'document', 'phone', 'address', 'number', 'neighborhood', 'city', 'state', 'zip_code', 'reference', 'status'];
  if (method === 'GET') { const result = table === 'freight_entries' ? await pool.query('SELECT f.*, d.name AS driver_name, v.plate, c.name AS client_name FROM freight_entries f LEFT JOIN drivers d ON d.id=f.driver_id LEFT JOIN vehicles v ON v.id=f.vehicle_id LEFT JOIN clients c ON c.id=f.client_id ORDER BY f.created_at DESC') : table === 'fuelings' ? await pool.query('SELECT f.*, v.plate, d.name AS driver_name, s.name AS station_name FROM fuelings f LEFT JOIN vehicles v ON v.id=f.vehicle_id LEFT JOIN drivers d ON d.id=f.driver_id LEFT JOIN partner_stations s ON s.id=f.station_id ORDER BY f.created_at DESC') : table === 'maintenance_entries' ? await pool.query('SELECT m.*, v.plate, v.model, s.name AS supplier_name FROM maintenance_entries m LEFT JOIN vehicles v ON v.id=m.vehicle_id LEFT JOIN suppliers s ON s.id=m.supplier_id ORDER BY m.created_at DESC') : await pool.query(`SELECT * FROM ${table} ORDER BY created_at DESC`); return json(response, 200, result.rows); }
  if (method === 'DELETE' && id) {
    try {
